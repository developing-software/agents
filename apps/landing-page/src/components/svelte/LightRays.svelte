...
void main() {
  ...
  gl_FragColor  = color;
      ...
      const updatePlacement = () => {
        ...
        (uniforms!.rayPos as { value: [number, number] }).value = anchor as [
          number,
          number,
        ];
        (uniforms!.rayDir as { value: [number, number] }).value = dir as [
          number,
          number,
        ];
      };
      ...
      const loop = (t: number) => {
        ...
        (uniforms.raysColor as { value: [number, number, number] }).value =
          hexToRgb(current.raysColor);
        ...
        (uniforms.pulsating as { value: number }).value = current.pulsating
          ? 1
          : 0;
        (uniforms.fadeDistance as { value: number }).value =
          current.fadeDistance;
        (uniforms.saturation as { value: number }).value = current.saturation;
        (uniforms.mouseInfluence as { value: number }).value =
          current.mouseInfluence;
        ...
        if (current.followMouse && current.mouseInfluence > 0) {
          ...
          (uniforms.mousePos as { value: [number, number] }).value = [
            smoothMouse.x,
            smoothMouse.y,
          ];
        }
...
<div
  ...
  class="pointer-events-none relative z-[3] h-full w-full overflow-hidden {className}"
></div>
