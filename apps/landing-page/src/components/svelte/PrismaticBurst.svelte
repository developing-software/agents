<script lang="ts">
  ...
  // Raymarch step count — the dominant per-pixel cost. Lower = faster, with a
  // softer/less detailed burst. Baked into the shader at compile time.
...
void main() {
  ...
  onMount(() => {
    // Render below native resolution — the burst is soft/glowy, so the upscale
    // is barely visible but cuts fragment-shader work quadratically.
    const dpr =
      Math.min(window.devicePixelRatio || 1, maxDpr) * Math.max(0.1, renderScale);
    ...
    gl.canvas.style.mixBlendMode =
      current.mixBlendMode && current.mixBlendMode !== "none"
        ? current.mixBlendMode
        : "";
    ...
    const applyProps = () => {
      ...
      program.uniforms.uAnimType.value =
        animTypeMap[current.animationType ?? "rotate"];
      program.uniforms.uDistort.value =
        typeof current.distort === "number" ? current.distort : 0;
      program.uniforms.uOffset.value = [
        toPx(current.offset?.x),
        toPx(current.offset?.y),
      ];
      program.uniforms.uRayCount.value = Math.max(
        0,
        Math.floor(current.rayCount ?? 0),
      );
      gl.canvas.style.mixBlendMode =
        current.mixBlendMode && current.mixBlendMode !== "none"
          ? current.mixBlendMode
          : "";
      // Only rebuild/upload the gradient texture when the colors change,
      // instead of allocating + uploading it on every frame.
    ...
    const update = (now: number) => {
      ...
      last = now;
      // Advance animation time every tick so motion speed stays independent of
      // the (capped) render cadence.
      if (!current.paused) accumTime += dt;

      const visible = isVisible && !document.hidden;
      if (!visible) return;
      // Throttle GPU work to the target frame rate (skip high-refresh excess).
      if (now - lastRender < frameInterval) return;
      const rdt = Math.max(0, now - lastRender) * 0.001;
      lastRender = now;

      applyProps();
      const tau =
        0.02 + Math.max(0, Math.min(1, current.hoverDampness ?? 0)) * 0.5;
      const alpha = 1 - Math.exp(-rdt / tau);
